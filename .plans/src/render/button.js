...
const PROFILE_CONFIG = {
    ...
    const bgIcon = Assets.cache.get(asset) || Assets.cache.get(id);
    ...
    if (config.frameTexture) {
        // Prioritize btnDef override, then config override, then default icon color
        let fColor = color;
        ...
        button.addTextureFrame(Assets.cache.get(config.frameTexture), fColor);
    }
    ...
    if (config.tagTexture) {
        ...
        button.addTag(Assets.cache.get(config.tagTexture), tColor);
    }
