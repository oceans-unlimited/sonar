...
*   **`submarine.submarineStateData.MOVED.xoChargedGauge`**: A boolean flag tracking if the XO has performed their mandatory charge action for the current move. When both this and `engineerCrossedOutSystem` are `true`, the sub returns to `SUBMERGED`.
...
### `ready_interrupt`
*   **Trigger**: Clicking 'Ready' during a Pause, Damage Resolution, or Sonar Resolution phase.
*   **Logic**: Signals the server that the XO is ready. When the `ready` array contains all required Player IDs, the game phase returns to `LIVE`.
