...
*   **`submarine.past_track`**: An array of `{ row, col }` objects tracking every position the submarine has visited since its last surfacing.
*   **`submarine.submarineState`**: Tracks if the vessel is `SUBMERGED`, `MOVED`, `SURFACING`, `SURFACED`, or `DESTROYED`.
...
### `ready_interrupt`
*   **Trigger**: Captain clicks "Ready" on an interrupt overlay (e.g., after a Torpedo impact or during a Pause).
*   **Logic**: Signals the server that the Captain is ready to resume.
